...
async function runTests() {
  ...
  (SecurityToolExecutor as any).execute = async (tool: string, action: string, args: any, cmd: string, cb: any, executeImpl: any) => {
    ...
    return { success: true, output: "Mocked Tauri IPC" };
  };
  ...
  try {
    ...
    const legacyResult = await AIToolExecutor.executeCapability(
      CapabilityRegistry.get('legacy_test_tool')!,
      { arg1: "test" },
      context
    );
    assert(securityExecutedTool === 'legacy_test_tool', "Legacy tool must route through SecurityToolExecutor");
    assert(legacyResult.success, "Legacy tool succeeded. Output: " + legacyResult.output);
    assert(legacyResult.output === "Mocked Tauri IPC", "Legacy tool returned mocked output");

    // Test Native via AIToolExecutor
    const nativeResult = await AIToolExecutor.executeCapability(
      CapabilityRegistry.get('builtin.ping')!,
      {},
      context
    );
