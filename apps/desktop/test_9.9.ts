...
import type { Plan, PlanStep, PlanEvent } from './src/lib/ai/types.js';
...
async function runTests() {
  ...
  let onApprovalRequested: (() => void) | null = null;
  ...
  planStateMachine.setEventHandler((evt: PlanEvent) => {
    events.push(evt);
  });
  ...
  try {
    ...
    FileOperations.createFolder = async (p: string) => {
      activeOps++; maxActiveOps = Math.max(maxActiveOps, activeOps);
    ...
    assert((ResourceLockManager as any).activeLocks.length === 0, 'No leaked locks after successful workflows');
    ...
    assert((ResourceLockManager as any).activeLocks.length === 0, 'No leaked locks after failed workflow');
    ...
    onApprovalRequested = () => {
      ...
      assert((ResourceLockManager as any).activeLocks.length === 0, 'Locks should be released during WAITING_FOR_USER');
    };
    ...
    assert((ResourceLockManager as any).activeLocks.length === 0, 'No leaked locks after cancellation');
