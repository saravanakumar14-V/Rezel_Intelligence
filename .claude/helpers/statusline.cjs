...
// Dynamically resolve CLI installation root at runtime.
// Checks environment variable override or candidate paths; falls back cleanly to null.
function resolveInstallRoot() {
  if (process.env.RUFLO_INSTALL_ROOT) return process.env.RUFLO_INSTALL_ROOT;
  try {
    const candidates = resolveCliBinCandidates();
    if (candidates && candidates.length > 0) {
      return path.resolve(path.dirname(candidates[0]), '..');
    }
  } catch { /* ignore */ }
  return null;
}
const BAKED_INSTALL_ROOT = resolveInstallRoot();
