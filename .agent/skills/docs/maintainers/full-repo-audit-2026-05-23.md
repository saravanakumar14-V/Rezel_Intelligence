...
- Evidence: secret-pattern scanning found `<AWS_ACCESS_KEY_EXAMPLE>`, and `<PRIVATE_KEY_HEADER>` examples. These are not confirmed live secrets: the AWS values are standard examples and the private key block is an ellipsis placeholder.
